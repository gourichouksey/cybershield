...
class APKAnalyzer {
  ...
  async parseManifest(apkPath) {
    ...
    try {
      ...
      console.info("Parsed manifest:", manifest);
  ...
  async parseAndroidManifest(apkPath) {
    ...
    if (ApkReader) {
      try {
        ...
        console.log(manifest.application.services)
        return {
            ...
            zipfile2.on('entry', (entry) => {
              if (entry.fileName.endsWith('.dex')) {
                zipfile2.openReadStream(entry, (err, readStream) => {
                  if (err) {
                    processedFiles++;
                    if (processedFiles === totalDexFiles) resolve(dexFiles);
                    return;
                  }
    ...
    try {
      ...
      matches.forEach(str => {
        ...
        if (lowerStr.includes('malware') || lowerStr.includes('trojan') || 
            lowerStr.includes('keylog') || lowerStr.includes('steal')) {
